...
//
//     npx vitest run --config vitest.live.config.ts tests/live/gamco3.livetest.ts
...
describe('GAMCO boundary wall — live run 3, truth-grounded answers', () => {
  it('completes the schedule with the client answers applied', async () => {
    ...
    const out = await runOrchestrator({
      ...
      objective:
        ...
        'legibly dimension (the column heights, SC height, F1 depth, wall height), point the ' +
        "dimension conclusion at the matching user fact with a {kind:'user-fact', factId:'...'} " +
    ...
    writeFileSync(path.join(runDir, `gamco3-${stamp}.log`), lines.join('\n'), 'utf8');
    writeFileSync(path.join(runDir, `gamco3-${stamp}.result.json`), JSON.stringify(out.result, null, 2), 'utf8');
    log(`saved: bbs-runs/gamco3-${stamp}.{log,result.json}`);
